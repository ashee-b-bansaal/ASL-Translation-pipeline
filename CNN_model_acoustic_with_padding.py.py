import sys
import os
# Ensure we import from the current directory (Final_1/)
...
parser.add_argument("--gap_threshold", type=float, default=None, help="Gap threshold factor (e.g., 1.0, 1.2, 1.5, 1.7, 2.0). If provided, uses diff_new_{threshold} and non_diff_new_{threshold} folders")
...
# Base path for results - will be constructed after mode configuration
# (moved below to use actual window_size and window_step from mode config)
...
if mode == 'signs':
    ...
    poc = [3]
    batch_size = 30
    window_size, window_step =220, 110
...
for i, p in enumerate(data_sessions):
    ...
    this_all_data, this_all_data_truth = read_from_folder(p, data_path, is_train=True, is_shuffle = True, seed_num = data_sessions[i], use_timestamps=False, gap_threshold_factor=gap_threshold)
