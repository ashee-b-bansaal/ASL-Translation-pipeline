...
def remove_emotions_from_gloss(gloss: str) -> str:
    """
    Preserve emotion expressions in a gloss string.
    This version keeps the gloss unchanged (no emotion removal).
    """
    return gloss
...
def filter_expressions(segments: List[Segment]) -> List[Segment]:
    """
    STEP 2: Normalize expression segments.
    
    Keeps all expressions (including emotions) and
    handles comma-separated tokens (e.g., "shake,cha").
...
def load_and_align_files(signs_file: str, exp_file: str, output_dir: str):
    ...
    results = []
    exact_matches = 0
    # Keep exact match count (ground-truth with emotions preserved)
    ...
    # Output file path
    output_file = output_dir_path / "alignment_without_LM_v2.txt"
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Simple Exact Match Accuracy: {exact_matches}/{len(results)} ({100*exact_match_rate:.2f}%)\n")
    ...
    with open(output_file_with_emotions, 'w', encoding='utf-8') as f:
        for idx, result in enumerate(results, 1):
            # Format: ground truth (with emotions) - aligned sentence --- ground truth emotion - recognized emotion
